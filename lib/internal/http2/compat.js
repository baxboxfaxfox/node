...
function isPseudoHeader(name) {
  return validPseudoHeaderNames.includes(name);
}
...
function onStreamData(chunk) {
  ...
  if (request !== undefined && request.push(chunk) === 0)
    this.pause();
}
...
function onStreamAbortedRequest() {
  const request = this[kRequest];
  if (request !== undefined && !request[kState].closed) {
